...
@Post("/signup")
public async userSignup(@Body() reqBody: IUserSignupDto) : Promise<IServerResponse<IAccessTokenData>>{
...
@Post("/signin")
public async userSignin(@Body() reqBody: ILoginDto) : Promise<IServerResponse<IAccessTokenData>>{
    const {emailAddress, password} = reqBody
    const connection = await getFreshConnection()
    const userRepo = connection.getRepository(User)
    const doesUserExist: User = await userRepo.findOne({ emailAddress: emailAddress })
    if(!doesUserExist){
